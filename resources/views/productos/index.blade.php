<x-layouts::app title="Productos">
    ...
    <form method="GET" action="{{ route('productos.index') }}" class="mb-6">
        <div class="bg-neutral-primary-soft border rounded-xl rounded-base p-4">
            <div class="grid grid-cols-2 md:grid-cols-2 gap-4">
                ...
                <flux:select name="stock">
                    <flux:select.option value="">Cualquier stock</flux:select.option>
                    <flux:select.option value="con" :selected="request('stock') == 'con'">Con stock</flux:select.option>
                    <flux:select.option value="sin" :selected="request('stock') == 'sin'">Sin stock</flux:select.option>
                    <flux:select.option value="poco" :selected="request('stock') == 'poco'">Poco stock (menos de 50)</flux:select.option>
                </flux:select>
    ...
    <div class="flex justify-between items-center mb-2">
        <a href="{{ route('productos.index', array_merge(request()->query(), ['order' => request('order', 'desc') == 'desc' ? 'asc' : 'desc'])) }}"
    ...
    <div class="relative overflow-x-auto bg-neutral-primary-soft shadow-xs rounded-base border rounded-xl mb-4">
        @if (count($productos) == 0)
            <p class="text-body text-sm">No se han encontrado productos.</p>
        @else
