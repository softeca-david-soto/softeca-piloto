<x-layouts::app>
    ...
    <form action="{{ route('clientes.update', $cliente) }} " method="POST" class="space-y-4" enctype="multipart/form-data">
        ...
        <div class="space-y-4">
			<flux:input name="name" placeholder="David Soto" label="Nombre" value="{{ old('name', $cliente->name) }}"></flux:input>
			<flux:input name="email" placeholder="davidsoto@mail.es" label="Email" value="{{ old('email', $cliente->email) }}"></flux:input>
			<flux:input name="phone" placeholder="645251575" label="Telefono" value="{{ old('phone', $cliente->phone) }}"></flux:input>
			...
			<flux:input name="zipcode" placeholder="09007" label="Codigo Postal" value="{{ old('zipcode', $cliente->zipcode) }}"></flux:input>
